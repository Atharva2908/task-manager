  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== 'admin' && data.user.role !== 'manager') {
        setError('This account does not have admin access. Please use the employee login.')
        setLoading(false)
        return
      }
      ...
      <div className="relative z-10 w-full max-w-md">
        <Card className="p-8 bg-slate-800 border-slate-700 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <Input
                ...
                className="bg-slate-700 border-slate-600 text-white placeholder-slate-500 focus:border-blue-500 focus:ring-blue-500/20"
                required
              />
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-700 text-center space-y-3">
            <Link href="/auth/forgot-password" className="block text-sm text-blue-400 hover:text-blue-300 transition">
              Forgot password?
            </Link>
            <div className="text-slate-400 text-sm">
              Not an admin?{' '}
              <Link href="/auth/employee-login" className="text-blue-400 hover:text-blue-300">
                Employee login
              </Link>
            </div>
